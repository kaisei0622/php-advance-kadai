...
if (isset($_POST['submit'])) {
	try {
			...
			$stmt_update->bindValue(':books_code', $_POST['books_code'], PDO::PARAM_INT);
			$stmt_update->bindValue(':books_name', $_POST['books_name'], PDO::PARAM_STR);
		...
		<article class="registration">
			...
			<form action="update.php?id=<?= $_GET['id'] ?>" method="post" class="registration-form">
				<div>
					...
					<input type="number" name="books_code" value="<?= $books['books_code'] ?>" min="0" max="100000000" required>

					<label for="books_name">書籍名</label>
					<input type="text" name="books_name" value="<?= $books['books_name'] ?>" maxlength="50" required>
