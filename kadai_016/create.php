...
if (isset($_POST['submit'])) {
	try {
		...
		$sql_insert = '
					INSERT INTO books (books_code, books_name, price, stock_quantity, genre_code)
					VALUES (:books_code, :books_name, :price, :stock_quantity, :genre_code)
			';
		$stmt_insert = $pdo->prepare($sql_insert);

		// bindValue()メソッドを使って実際の値をプレースホルダにバインドする（割り当てる）
		$stmt_insert->bindValue(':books_code', $_POST['books_code'], PDO::PARAM_INT);
		$stmt_insert->bindValue(':books_name', $_POST['books_name'], PDO::PARAM_STR);
	...
	<main>
		<article class="registration">
			...
			<form action="create.php" method="post" class="registration-form">
				<div>
					<label for="books_code">書籍コード</label>
					<input type="number" name="books_code" min="0" max="100000000" required>

					<label for="books_name">書籍名</label>
					<input type="text" name="books_name" maxlength="50" required>
